...
def getTop10Labels(df):
    df_label = df.explode('label')
    label_count = df_label.groupby(
        "label")['label'].count().reset_index(name="count")
    label_count = label_count.sort_values(by=['count'], ascending=False)
    return label_count
...
def getIssuesByRelatedLabel(df, selection):
    ...
    df['val'] = df['label'].apply(
        lambda x: all(y in str(x) for y in selection))
    df = df[df['val'] == True]
    return df
...
def indexByDateRange(df, start, end=dt.strftime(dt.now(), '%Y-%m-%d')):
    ...
    df['created_at'] = df['created_at'].apply(
        lambda x: x.strftime('%Y-%m-%d')).astype(str)
...
def numofIssuesByDate(df):
    ...
    groupOpen = openIssues.groupby(
        "date")['url'].count().reset_index(name="open")
    groupClosed = closedIssues.groupby(
        "date")['url'].count().reset_index(name="closed")
...
def getIssuesStacked(df):
    ...
    df = df.explode("label")
    df = df.groupby(["label", "status"])[
        'url'].count().reset_index(name="count")
